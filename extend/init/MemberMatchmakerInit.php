 ...
 *     "version"         =>"1.0",
 *     "use"             => new \init\MemberMatchmakerInit();
 * )
 */
...
class MemberMatchmakerInit extends Base
{

    public $status    = [1 => '正常', 2 => '已过期'];//状态
    public $is_manage = [1 => '未充值', 2 => '正常', 3 => '已到期'];//管理
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        if ($this->InterfaceType == 'api') {
            //api处理文件
            if ($item['wx_image']) $item['wx_image'] = cmf_get_asset_url($item['wx_image']);
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($params["id"])) {
            ...
            $result                = $MemberMatchmakerModel->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } elseif (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $MemberMatchmakerModel->where($where)->strict(false)->update($params);
            if ($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $MemberMatchmakerModel->strict(false)->insert($params, true);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $MemberMatchmakerModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $MemberMatchmakerModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $MemberMatchmakerModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $MemberMatchmakerModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }

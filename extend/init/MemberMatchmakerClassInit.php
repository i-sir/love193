...
/**
    ...
    *     "version"         =>"1.0",
    *     "use"             => new \init\MemberMatchmakerClassInit();
    * )
    */
...
class MemberMatchmakerClassInit extends Base
{
    ...
    public $DataFormat    = "find";//数据格式,find详情,list列表
        ...
        */
        public function get_list($where=[], $params = [])
        {
            ...
            /** 查询数据 **/
            $result = $MemberMatchmakerClassModel
            ->where($where)
    ...
    public function get_join_list($where=[], $params = [])
    {
        ...
        */
        public function get_find($where=[],$params=[])
        {
            ...
            /** 查询数据 **/
            $item = $MemberMatchmakerClassModel
                ->where($where)
                ->order($params['order'] ?? $this->Order)
                ->field($params['field'] ?? $this->Field)
                ->find();
        ...
        */
        public function edit_post($params,$where=[])
        {
            ...
            if (!empty($params["id"])) {
                ...
                $result                = $MemberMatchmakerClassModel->where("id","=",$params["id"])->strict(false)->update($params);
                if($result) $result = $item["id"];
            }elseif(!empty($where)){
                //传入where条件,根据条件更新数据
                $params["update_time"] = time();
                $result                = $MemberMatchmakerClassModel->where($where)->strict(false)->update($params);
                if ($result) $result = $item["id"];
            }else {
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $MemberMatchmakerClassModel->strict(false)->insert($params,true);
            }
        ...
        */
        public function edit_post_two($params,$where=[])
        {
            ...
            if (!empty($params["id"])) {
                //如传入id,根据id编辑数据
                $params["update_time"] = time();
                $result                = $MemberMatchmakerClassModel->strict(false)->update($params);
                if($result) $result = $item["id"];
            } elseif(!empty($where)){
                //传入where条件,根据条件更新数据
                $params["update_time"] = time();
        ...
        */
        ...
        if ($type == 1) $result = $MemberMatchmakerClassModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $MemberMatchmakerClassModel->destroy($id,true);//真实删除
        ...
        */
        public function batch_post($id, $params=[])
        {
            ...
            $params["update_time"] = time();
            $result = $MemberMatchmakerClassModel->where($where)->strict(false)->update($params);//修改状态
        ...
        */
        public function list_order_post($list_order,$params=[])
        {
            ...
            foreach ($list_order as $k => $v) {
                $where   = [];
                $where[] = ["id", "=", $k];
                $result  = $MemberMatchmakerClassModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
            }
